<template>
  <div ref="cropper" class="vue-cropper" @mouseover="scaleImg" @mouseout="cancelScale">
    ...
    >
      ...
      <span v-if="info" class="crop-info" :style="{ top: cropInfo.top }"
    ...
    if (!HTMLCanvasElement.prototype.toBlob) {
      Object.defineProperty(HTMLCanvasElement.prototype, 'toBlob', {
        value: function (callback, type, quality) {
          ...
          callback(new Blob([arr], { type: _type || 'image/png' }));
        },
      });
    }
    this.showPreview();
      ...
      this.$nextTick(() => {
        if (this.centerBox) {
          ...
          switch (this.rotate) {
            ...
            case -3:
              maxLeft =
                this.cropOffsertX - (this.trueWidth * (1 - this.scale)) / 2 + (imgW - imgH) / 2;
              maxTop =
                this.cropOffsertY - (this.trueHeight * (1 - this.scale)) / 2 + (imgH - imgW) / 2;
    ...
    createCrop(e) {
      ...
      this.$nextTick(() => {
        ...
        if (fw > 0) {
          ...
          this.cropW =
            this.w - this.cropChangeX + Math.abs(fw) > this.w ? this.cropChangeX : Math.abs(fw);
        ...
        if (!this.fixed) {
          if (fh > 0) {
            ...
            this.cropH =
              this.h - this.cropChangeY + Math.abs(fh) > this.h ? this.cropChangeY : Math.abs(fh);
    ...
    changeCropNow(e) {
      ...
      this.$nextTick(() => {
        ...
        if (this.canChangeX) {
          if (this.changeCropTypeX === 1) {
            if (this.cropOldW - fw > 0) {
              ...
              this.cropOffsertX =
                wrapperW - this.cropChangeX - fw <= wrapperW - minX ? this.cropChangeX + fw : minX;
            } else {
              this.cropW =
        ...
        if (this.canChangeY) {
          if (this.changeCropTypeY === 1) {
            if (this.cropOldH - fh > 0) {
              ...
              this.cropOffsertY =
                wrapperH - this.cropChangeY - fh <= wrapperH - minY ? this.cropChangeY + fh : minY;
            } else {
              this.cropH =
        ...
        if (this.canChangeY && this.fixed) {
          let fixedWidth = (this.cropH / this.fixedNumber[1]) * this.fixedNumber[0];
